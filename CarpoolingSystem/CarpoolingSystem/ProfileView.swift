...
struct ProfileView: View {
    ...
    @State private var showWallet = false  // ✅ 钱包显示状态
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showWallet) {
            if let user = authManager.currentUser {
                WalletView(
                    viewModel: FinalPassengerViewModel(
                        userID: user.id ?? "",
                        userName: user.name,
    ...
    /// 用户信息头部
    private func userHeaderSection(user: AppUser) -> some View {
        VStack(spacing: 16) {
            // 用户头像
            ZStack {
                Circle()
    ...
    /// 功能菜单区域
    private func menuSection(user: AppUser) -> some View {
        VStack(spacing: 16) {
            // ✅ 钱包功能（新增）
            MenuGroupCard(title: "财务管理") {
                Button(action: {
                    showWallet = true
                }) {
                    HStack(spacing: 16) {
...
struct CreateProfileView: View {
    ...
    var isFormValid: Bool {
        !name.isEmpty && !phone.isEmpty && (selectedRole == .passenger || !carPlate.isEmpty)
    }
...
struct ProfileView_PreviewWrapper: View {
    ...
    init() {
        ...
        authManager.currentUser = AppUser(
            ...
            rating: 4.8,
            completedRides: 15,
            joinDate: Date().addingTimeInterval(-60 * 60 * 24 * 30),
            role: .carOwner,
